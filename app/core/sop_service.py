...
class SOPService:
    ...
    async def create_sop(self, request: CreateSOPRequest) -> CreateSOPResponse:
        ...
        """
        try:
            self.logger.info("Creating new SOP record",
            ...
            self.logger.error("Error creating SOP record",
                            sop_name=request.sop_name,
                            error=str(e))
            raise

    async def update_sop(self, sop_id: str, request: UpdateSOPRequest) -> Optional[UpdateSOPResponse]:
        """
