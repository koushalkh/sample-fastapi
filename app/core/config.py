...
class Settings:
    ...
    def __init__(self) -> None:
        ...
        self.secrets_manager_region = os.getenv("SECRETS_MANAGER_REGION", self.aws_region)
...
def _load_from_environment(settings: Settings) -> None:
    ...
    settings.set_horizon_config({
        ...
        "HORIZON_API_SECURE": os.getenv("HORIZON_API_SECURE", "true").lower() == "true"
    })
    ...
    settings.set_dynamodb_secrets({
        "ADR-DynamoNamespace": os.getenv("ADR_DYNAMO_NAMESPACE", f"{settings.app_name.lower()}-dynamodb-local"),
...
def _load_from_secrets_manager(settings: Settings) -> None:
    ...
    if "HORIZON_API_SECURE" in horizon_config:
        horizon_config["HORIZON_API_SECURE"] = str(horizon_config["HORIZON_API_SECURE"]).lower() == "true"
...
def _get_config_with_fallback(
    ...
    if secret_data:
        logger.debug(f"Successfully loaded {config_name} configuration from AWS Secrets Manager",
                    secret_name=secret_name)
        return secret_data
    
    # Fallback to environment variables
    logger.info(f"Falling back to environment variables for {config_name} configuration",
               secret_name=secret_name)
    ...
    if missing_vars:
        logger.warning(f"Missing environment variables for {config_name} configuration",
