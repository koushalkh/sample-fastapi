...
class SOPModel(BaseModel):
    ...
    abend_type: str = Field(
        ..., alias="abendType", description="Type of abend this SOP addresses"
    )
    created_at: datetime = Field(
        ..., alias="createdAt", description="SOP creation timestamp"
    )
    updated_at: datetime = Field(
        ..., alias="updatedAt", description="SOP last update timestamp"
    )
...
class SOPDetailsModel(BaseModel):
    ...
    source_document_url: str = Field(
        ...,
        alias="sourceDocumentUrl",
        description="S3 URL of the original SOP document",
    )
    processed_document_urls: List[str] = Field(
        default=[],
        alias="processedDocumentUrls",
        description="S3 URLs of processed document versions",
    )
    ...
    created_by: str = Field(
        default="system", alias="createdBy", description="Who created this SOP"
    )
    updated_by: str = Field(
        default="system", alias="updatedBy", description="Who last updated this SOP"
    )
    generation: int = Field(
        default=1, description="Version number for optimistic locking"
    )
...
class CreateSOPRequest(BaseModel):
    ...
    job_name: str = Field(
        ...,
        alias="jobName",
        description="Associated job name",
        min_length=1,
        max_length=255,
    )
    abend_type: str = Field(
        ...,
        alias="abendType",
        description="Type of abend this SOP addresses",
        min_length=1,
        max_length=100,
    )
    source_document_url: str = Field(
    ...
    processed_document_urls: Optional[List[str]] = Field(
        None,
        alias="processedDocumentUrls",
        description="S3 URLs of processed document versions",
    )
    created_by: Optional[str] = Field(
        None, alias="createdBy", description="Who is creating this SOP", max_length=255
    )
...
class UpdateSOPRequest(BaseModel):
    ...
    updated_by: Optional[str] = Field(
        None, alias="updatedBy", description="Who is updating this SOP", max_length=255
    )
...
class GetSOPsFilter(BaseModel):
    ...
    job_name: Optional[str] = Field(
        None, alias="jobName", description="Filter by job name"
    )
    abend_type: Optional[str] = Field(
        None, alias="abendType", description="Filter by abend type"
    )
    search: Optional[str] = Field(None, description="Search term for SOP name")
    limit: int = Field(
        default=20, description="Number of records to return (max 100)", le=100, ge=1
    )
...
class UpdateSOPResponse(BaseModel):
    ...
    updated_at: datetime = Field(
        ..., alias="updatedAt", description="Record update timestamp"
    )
