...
def get_abend_by_tracking_id(tracking_id: str) -> Optional[AbendDetailsModel]:
    ...
    try:
        ...
        abend_record = AbendDynamoTable.get(hash_key=tracking_id, range_key="ABEND")
    ...
    try:
        ...
        for date_index, date_str in enumerate(date_list):
            ...
            try:
                ...
                day_records = [
                    _convert_to_abend_model(record) for record in day_results
                ]
                ...
                logger.warning(
                    "Error querying date partition", date=date_str, error=str(e)
                )
                continue
        else:
        ...
        logger.info(
            "Retrieved ABENDs by date range",
...
def create_abend(
    ...
    try:
        ...
        logger.info(
            "Successfully created ABEND record",
            tracking_id=tracking_id,
            job_name=job_name,
            adr_status=ADRStatusEnum.ABEND_REGISTERED.value,
        )
...
def update_abend_fields(tracking_id: str, updates: Dict[str, Any]) -> bool:
    """
    ...
    try:
        logger.info(
            "Updating ABEND fields", tracking_id=tracking_id, field_count=len(updates)
        )
        ...
        logger.error("Error updating ABEND", tracking_id=tracking_id, error=str(e))
        raise
...
def get_job_history(
    ...
    try:
        ...
        if start_date and end_date:
            ...
            query_kwargs["range_key_condition"] = AbendDynamoTable.abended_at.between(
            ...
            query_kwargs["range_key_condition"] = (
                AbendDynamoTable.abended_at >= start_date_str
            )
        elif end_date:
            # Use <= condition for range key
            end_date_str = end_date.replace(microsecond=0).isoformat()
            query_kwargs["range_key_condition"] = (
                AbendDynamoTable.abended_at <= end_date_str
            )
        ...
        logger.info("Retrieved job history", job_name=job_name, count=len(records))

        return records

    except Exception as e:
        logger.error("Error getting job history", job_name=job_name, error=str(e))
        raise
...
def get_stats_for_date(target_date: date) -> Dict[str, int]:
    ...
    try:
        ...
        results = AbendDynamoTable.abended_date_index.query(
            hash_key=date_str, scan_index_forward=False
        )
        ...
        stats = {
            ...
            "active_abends": status_counts.get("IN_PROGRESS", 0)
            + status_counts.get("PENDING", 0),
            "manual_intervention_required": status_counts.get(
...
def get_audit_logs_by_tracking_id(tracking_id: str) -> List[AuditLogModel]:
    ...
    try:
        ...
        logger.info(
            "Retrieved audit logs", tracking_id=tracking_id, count=len(audit_logs)
        )

        return audit_logs

    except Exception as e:
        logger.error("Error getting audit logs", tracking_id=tracking_id, error=str(e))
        raise
