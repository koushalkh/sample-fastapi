...
def get_sops_by_job_name(
    ...
    try:
        ...
        logger.info("SOPs retrieved by job name", 
                   job_name=job_name, 
                   count=len(records),
        ...
        logger.error("Error getting SOPs by job name", 
                    job_name=job_name, 
                    error=str(e))
        raise
...
def get_sops_by_abend_type(
    ...
    try:
        ...
        logger.info("SOPs retrieved by abend type", 
        ...
        logger.error("Error getting SOPs by abend type", 
                    abend_type=abend_type, 
                    error=str(e))
        raise
...
def get_sops_unified(
    ...
    """
    try:
        logger.info("Getting SOPs with unified query", 
                   job_name=job_name,
                   abend_type=abend_type,
        ...
        if job_name:
            # Use JobNameIndex
            records, next_key = get_sops_by_job_name(job_name, limit, last_evaluated_key)
        elif abend_type:
            # Use AbendTypeIndex  
            records, next_key = get_sops_by_abend_type(abend_type, limit, last_evaluated_key)
        else:
            # Scan table (less efficient, but needed for search-only queries)
            logger.warning("Performing table scan for SOP query - consider adding filters")
            records, next_key = _scan_sops_with_filters(search_term, limit, last_evaluated_key)
        ...
        if search_term and search_term.strip():
            ...
            filtered_records = [
                ...
                if search_lower in record.sop_name.lower()
            ]
            records = filtered_records
        
        logger.info("Unified SOP query completed", 
                   count=len(records),
...
def create_sop(
    ...
    logger.info("Creating new SOP record",
                sop_id=sop_id,
                sop_name=sop_name,
                job_name=job_name,
                abend_type=abend_type)
    ...
    try:
        ...
        logger.info("Successfully created SOP record",
                   sop_id=sop_id,
                   sop_name=sop_name,
                   job_name=job_name)
        ...
        logger.error("Error creating SOP record",
                    sop_id=sop_id,
                    sop_name=sop_name,
                    error=str(e))
        raise
...
def _convert_to_sop_details_model(record: SOPDynamoTable) -> SOPDetailsModel:
    ...
    return SOPDetailsModel(
        ...
        processedDocumentUrls=list(record.processed_document_urls) if record.processed_document_urls else [],
