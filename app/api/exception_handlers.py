...
async def validation_exception_handler(request: Request, exc: ValidationError) -> JSONResponse:
    ...
    field = first_error.get('loc', ['unknown'])[-1]  # Get the last part of the field path
...
async def pydantic_core_validation_exception_handler(request: Request, exc: PydanticCoreValidationError) -> JSONResponse:
    """Handle Pydantic Core validation errors and return 400 instead of 422."""
    logger.warning("Pydantic core validation error", path=str(request.url), error=str(exc))
    ...
    if "validation error for" in error_msg:
        ...
        if len(parts) > 1:
            ...
            field = model_and_field.split(".")[-1] if "." in model_and_field else "unknown"
        else:
            field = "unknown"
    else:
        field = "unknown"
