...
async def create_audit_log(request: CreateAuditLogRequest) -> CreateAuditLogResponse:
    ...
    try:
        logger.info(
            "Received create audit log request via internal API",
        ...
        logger.info(
            "Successfully created audit log entry via internal API",
...
async def get_audit_logs_for_tracking_id(
    ...
    try:
        ...
        if len(filtered_logs) == 0:
            logger.warning(
                "No audit logs found for tracking ID via internal API",
                tracking_id=tracking_id,
            )
            raise HTTPException(
