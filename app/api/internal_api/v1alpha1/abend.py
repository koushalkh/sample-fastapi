...
async def create_audit_log(request: CreateAuditLogRequest) -> CreateAuditLogResponse:
    ...
    """
    try:
        logger.info("Received create audit log request via internal API",
        ...
        logger.info("Successfully created audit log entry via internal API",
        ...
        logger.error("Validation error creating audit log", 
...
async def get_audit_logs_for_tracking_id(
    tracking_id: str,
    level: Optional[AuditLevelEnum] = Query(None, description="Filter by audit log level"),
    adr_status: Optional[ADRStatusEnum] = Query(None, description="Filter by ADR status", alias="adrStatus"),
    limit: Optional[int] = Query(None, description="Maximum number of audit logs to return", ge=1, le=100)
) -> GetAuditLogsResponse:
    """
    ...
    """
    try:
        logger.info("Getting audit logs via internal API",
        ...
        if adr_status:
            filtered_logs = [log for log in filtered_logs if log.adr_status == adr_status]
        ...
        logger.info("Successfully retrieved audit logs via internal API",
                   tracking_id=tracking_id,
                   total_count=len(filtered_logs),
                   original_count=len(response.audit_logs))
        
        if len(filtered_logs) == 0:
            logger.warning("No audit logs found for tracking ID via internal API", 
                         tracking_id=tracking_id)
            raise HTTPException(
        ...
        logger.error("Error retrieving audit logs via internal API", 
                    tracking_id=tracking_id,
                    error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
