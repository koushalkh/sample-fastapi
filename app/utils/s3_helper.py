...
def get_log_content(s3_uri: str) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
    ...
    try:
        ...
        logger.error("Unexpected error retrieving S3 content", error=str(e), s3_uri=s3_uri)
        return None, None
...
def check_file_exists(s3_uri: str) -> bool:
    ...
    try:
        ...
        if error_code == '404':
            return False
        logger.error("Error checking S3 file existence", error_code=error_code, s3_uri=s3_uri)
        return False
...
def get_file_metadata(s3_uri: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error("Unexpected error getting S3 metadata", error=str(e), s3_uri=s3_uri)
        return None


def upload_file_content(content: str, s3_uri: str, content_type: str = 'text/plain') -> bool:
    """
...
def upload_file_from_path(file_path: str, s3_uri: str, content_type: Optional[str] = None) -> bool:
    """
    ...
    try:
        ...
        logger.info("Successfully uploaded file to S3", file_path=file_path, bucket=bucket, key=key)
        return True
        ...
        error_code = e.response['Error']['Code']
        logger.error("S3 upload error", error_code=error_code, file_path=file_path, s3_uri=s3_uri)
        return False
        
    except Exception as e:
        logger.error("Unexpected error uploading file to S3", error=str(e), file_path=file_path, s3_uri=s3_uri)
        return False
...
def move_file(source_s3_uri: str, destination_s3_uri: str) -> bool:
    ...
    if not all([source_bucket, source_key, dest_bucket, dest_key]):
        logger.error("Invalid S3 URI format", source=source_s3_uri, destination=destination_s3_uri)
        return False
    
    try:
        logger.info("Moving file in S3", source=source_s3_uri, destination=destination_s3_uri)
        ...
        logger.info("Successfully moved file in S3", source=source_s3_uri, destination=destination_s3_uri)
        return True
        
    except ClientError as e:
        error_code = e.response['Error']['Code']
        logger.error("S3 move error", error_code=error_code, source=source_s3_uri, destination=destination_s3_uri)
        return False
        
    except Exception as e:
        logger.error("Unexpected error moving file in S3", error=str(e), source=source_s3_uri, destination=destination_s3_uri)
        return False
...
def copy_file(source_s3_uri: str, destination_s3_uri: str) -> bool:
    ...
    try:
        logger.info("Copying file in S3", source=source_s3_uri, destination=destination_s3_uri)
        ...
        logger.info("Successfully copied file in S3", source=source_s3_uri, destination=destination_s3_uri)
        return True
        
    except ClientError as e:
        error_code = e.response['Error']['Code']
        logger.error("S3 copy error", error_code=error_code, source=source_s3_uri, destination=destination_s3_uri)
        return False
        
    except Exception as e:
        logger.error("Unexpected error copying file in S3", error=str(e), source=source_s3_uri, destination=destination_s3_uri)
        return False
...
def delete_file(s3_uri: str) -> bool:
    ...
    try:
        ...
        logger.error("Unexpected error deleting file from S3", error=str(e), s3_uri=s3_uri)
        return False
