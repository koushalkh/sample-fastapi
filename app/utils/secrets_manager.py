...
class SecretsManager:
    ...
    def client(self):
        ...
        if self._client is None:
            try:
                ...
                self.logger.debug("AWS Secrets Manager client initialized", region=self.region_name)
            except Exception as e:
                self.logger.warning("Failed to initialize AWS Secrets Manager client", 
                                  region=self.region_name, error=str(e))
    ...
    def get_secret(self, secret_name: str) -> Optional[Dict[str, Any]]:
        ...
        """
        if not self.client:
            self.logger.debug("AWS Secrets Manager client not available", 
        ...
        try:
            ...
            if not secret_string:
                self.logger.warning("Secret exists but has no SecretString", 
                                  secret_name=secret_name)
                return None
            ...
            try:
                ...
                self.logger.debug("Successfully parsed secret JSON", 
                                secret_name=secret_name, 
            ...
            if error_code == 'ResourceNotFoundException':
                self.logger.warning("Secret not found in AWS Secrets Manager", 
                                  secret_name=secret_name)
            elif error_code == 'InvalidRequestException':
                self.logger.error("Invalid request to AWS Secrets Manager", 
                                secret_name=secret_name, error=str(e))
            elif error_code == 'InvalidParameterException':
                self.logger.error("Invalid parameter for AWS Secrets Manager", 
                                secret_name=secret_name, error=str(e))
            else:
                self.logger.error("AWS Secrets Manager client error", 
                                secret_name=secret_name, error_code=error_code, error=str(e))
            return None
            
        except BotoCoreError as e:
            self.logger.error("BotoCore error accessing AWS Secrets Manager", 
                            secret_name=secret_name, error=str(e))
            return None
            
        except Exception as e:
            self.logger.error("Unexpected error accessing AWS Secrets Manager", 
                            secret_name=secret_name, error=str(e))
            return None
